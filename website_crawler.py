...
class WebCrawler():
    ...
    def find_links_to_websites_containing_html_keyword(self, urls: List[str], content_keywords: List[str] = None, url_keywords: List[str] = None, bad_link_prefixes: List[str] = None, bad_link_postfixes: List[str] = None, reconnection_trys: int = 2, known_good_links: List[str] = [], known_bad_links: List[str] = []) -> Tuple[List[str], List[str]]:
                ...
                try:                
                    ...
                    for link_id, link in enumerate(local_links):
                        ...
                        if "href" in link.attrs:
                            ...
                            if valid and unknown and content_keywords is not None:
                                try:
                                    source_code = session.get(local_link)                    
                                    link_soup = BeautifulSoup(source_code.text, "html.parser")
                                            
                                    if all([s in str(link_soup) for s in content_keywords]):
                                        save_link = True
...
def crawl_links_to_keyword_sites(base_url: str, content_keyword: str, bad_link_prefixes: List[str] = None, bad_link_postfixes: List[str] = None, try_to_load_existing: bool = True, verbose: bool = False) -> pd.DataFrame:
    """This function crawls for links within the webiste of the 'base_url'. 
    ...
    if not os.path.exists(c.get_results_dir_name()):
        os.mkdir(c.get_results_dir_name())
    if not os.path.exists(c.get_url_list_path(base_url)):
        os.mkdir(c.get_url_list_path(base_url))
    
    if try_to_load_existing:
        fn: str = c.get_all_url_list_path(base_url)
        if os.path.exists(fn):
            url_list = np.load(fn, allow_pickle=True).tolist()
            print(f"Loaded {len(url_list)} many known hyperrefs from file")
    ...
    target_sites, no_target_sites = wc.find_links_to_websites_containing_html_keyword(url_list, content_keywords=content_keyword, bad_link_prefixes=bad_link_prefixes, bad_link_postfixes=bad_link_postfixes, known_good_links=known_positives, known_bad_links=known_negatives)
...
def crawl_compeititon_links_from_tournament_links():
    ...
    for tournament_link, tournament_id in unprocessed_df.itertuples(index=False):
        ...
        competition_link_list, _ = wc.find_links_to_websites_containing_html_keyword([tournament_link], url_keywords=c.C.URL_KEYWORDS.value)
...
if __name__ == "__main__":
    # # Update the list of tournaments.
    # url: str = "https://www.tanzsport.de/de/sportwelt/ergebnisse" # "https://www.tanzsportkreis-sankt-augustin.de"
    # crawl_links_to_keyword_sites(base_url=url, content_keyword=c.ColName.CONTENT_KEYWORDS, bad_link_prefixes=['mailto', 'tel', '#', '?s'], bad_link_postfixes=['.jpy', '.png'])

    # # Update the list of competition.
    # crawl_compeititon_links_from_tournament_links()
    pass